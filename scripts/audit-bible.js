...
async function runAudit() {
  ...
  console.log(`Bíblia Base (ACF) mapeada. Total de Capítulos: ${totalExpectedChapters} | Total de Versículos: ${totalExpectedVerses}`);
  ...
  const { data: chaptersData, error: chaptersError } = await supabase
    .rpc('get_distinct_chapters'); // we might not have this RPC, let's just fetch everything grouped if possible, or fetch all and process in memory since it's only ~31000 rows.
  ...
  for (const book of Object.keys(expectedStructure)) {
    ...
    for (let c = 1; c <= expBook.chapters; c++) {
      if (!foundBook[c]) {
        ...
        missingVerses.push(`Capítulo ${book} ${c} esperava ${expBook.versesPerChapter[c-1]} versos, encontrou ${foundBook[c]}`);
      }
    }
  }
  ...
  if (duplicates.length === 0 && missingChapters.length === 0 && missingVerses.length === 0 && invalidStructure.length === 0 && actualCount === totalExpectedVerses) {
