...
async function runAudit() {
  ...
  try {
    ...
    await supabase.from('time_capsules').delete().eq('user_id', testUserId).eq('month_year', '2000-01');
    ...
    const { data: readData, error: readErr } = await supabase.from('time_capsules')
      .select('*').eq('user_id', testUserId).eq('month_year', '2000-01').single();
    
    if (readErr || !readData) throw new Error("Erro na leitura após gravação");
    if (!readData.objectives || readData.objectives[0] !== 'Aprender') throw new Error("Coluna objectives falhou na expansão JSONB");
  ...
  try {
    ...
    const { data: note, error: insertErr } = await supabase.from('verse_notes')
      .insert({
        user_id: testUserId,
        version: 'GLOBAL',
        book: 'Gênesis',
    ...
    const { error: updateErr } = await supabase.from('verse_notes')
    ...
    const { data: history, error: histErr } = await supabase.from('verse_notes_history')
      .select('*').eq('note_id', note.id);

    if (histErr) throw new Error("Erro ao ler tabela verse_notes_history: " + histErr.message);
    if (!history || history.length === 0) throw new Error("O Trigger NÃO gravou a versão anterior na tabela de histórico.");
    if (history[0].content !== 'Versão 1') throw new Error(`O conteúdo salvo no histórico não confere (salvou: ${history[0].content}).`);

    console.log("✅ Histórico de Anotações e Triggers estão perfeitos (A versão anterior foi arquivada automaticamente).");
  ...
  try {
    ...
    const { error: uploadErr } = await supabase.storage.from('capsule-media').upload(testFileName, 'teste-123', {
      contentType: 'text/plain'
    });

    if (uploadErr) {
      if (uploadErr.message.includes('row-level security')) {
         throw new Error("Erro de RLS - As políticas do bucket podem estar impedindo o service_role ou o upload.");
      }
      throw new Error("Erro no upload: " + uploadErr.message);
    }

    const { data: { publicUrl } } = supabase.storage.from('capsule-media').getPublicUrl(testFileName);
    if (!publicUrl) throw new Error("Falha ao gerar URL pública.");

    console.log("✅ Supabase Storage operando 100%. Bucket 'capsule-media' existe e aceita uploads.");
