...
import {
  HeartHandshake, Flame, CheckCircle2, BookHeart, Plus, X, Check,
  Trash2, ChevronRight, Sparkles, Clock, BookOpen, Target, Heart,
  BookMarked, Lock, Eye, Calendar, Award, Edit2, Loader2
} from "lucide-react";
...
type PrayerKind = "prayer" | "devotional" | "purpose" | "fast" | "gratitude" | "learning" | "favorite_verse" | "timeline";
...
const SECTIONS = [
  { kind: "prayer" as PrayerKind, icon: HeartHandshake, label: "Orações", emoji: "🙏", placeholder: "Qual o seu pedido de oração?" },
  { kind: "devotional" as PrayerKind, icon: BookOpen, label: "Devocionais", emoji: "📖", placeholder: "Qual versículo leu hoje?" },
  { kind: "purpose" as PrayerKind, icon: Target, label: "Propósitos", emoji: "🎯", placeholder: "Qual o seu propósito?" },
  { kind: "fast" as PrayerKind, icon: Flame, label: "Jejuns", emoji: "🍞", placeholder: "Motivo do seu jejum?" },
  { kind: "gratitude" as PrayerKind, icon: Heart, label: "Gratidão", emoji: "❤️", placeholder: "Pelo que você é grato hoje?" },
  { kind: "learning" as PrayerKind, icon: BookHeart, label: "Aprendizados", emoji: "🌱", placeholder: "O que aprendeu?" },
  { kind: "favorite_verse" as PrayerKind, icon: BookMarked, label: "Favoritos", emoji: "📌", placeholder: "Referência do versículo (Ex: Romanos 8:28)" },
  { kind: "timeline" as PrayerKind, icon: Clock, label: "Linha do Tempo", emoji: "📅", placeholder: "" }
];
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-white/5 pb-2 mb-2">
        ...
        <button type="button" onClick={onClose} className="p-1 rounded-full hover:bg-white/10 text-white/40">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      {kind === "prayer" && (
        <div className="space-y-3">
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Título do Pedido</label>
            <input
              autoFocus
              value={title}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Detalhes / Motivo</label>
            <textarea
              value={description}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Categoria</label>
              <select
                value={category}
                onChange={e => setCategory(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none"
              >
                <option value="família" className="bg-[#17152b]">Família</option>
                <option value="saúde" className="bg-[#17152b]">Saúde</option>
                <option value="trabalho" className="bg-[#17152b]">Trabalho</option>
                <option value="estudos" className="bg-[#17152b]">Estudos</option>
                <option value="igreja" className="bg-[#17152b]">Igreja</option>
                <option value="outro" className="bg-[#17152b]">Outros</option>
              </select>
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Privacidade</label>
              <select
                value={privacy}
                onChange={e => setPrivacy(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none"
              >
                <option value="private" className="bg-[#17152b]">Somente eu (Privado)</option>
                <option value="public" className="bg-[#17152b]">Público (Comunhão)</option>
              </select>
            </div>
          </div>
        </div>
      ...
      {kind === "devotional" && (
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Versículo Lido</label>
              <input
                autoFocus
                value={verse}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Como se sentia?</label>
              <select
                value={feeling}
                onChange={e => setFeeling(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none"
              >
                <option value="😊 Alegre" className="bg-[#17152b]">😊 Alegre</option>
                <option value="🙏 Grato" className="bg-[#17152b]">🙏 Grato</option>
                <option value="😴 Cansado" className="bg-[#17152b]">😴 Cansado</option>
                <option value="🤔 Reflexivo" className="bg-[#17152b]">🤔 Reflexivo</option>
                <option value="😔 Ansioso" className="bg-[#17152b]">😔 Ansioso</option>
                <option value="💪 Fortalecido" className="bg-[#17152b]">💪 Fortalecido</option>
              </select>
            </div>
          </div>
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">O que aprendeu?</label>
            <textarea
              value={learned}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Como aplicar no dia a dia?</label>
            <textarea
              value={application}
      ...
      {kind === "purpose" && (
        <div className="space-y-3">
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Propósito / Compromisso</label>
            <input
              autoFocus
              value={title}
              onChange={e => setTitle(e.target.value)}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Objetivo Geral</label>
            <textarea
              value={objective}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Data de Início</label>
              <input
                type="date"
                value={startDate}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Prazo (Opcional)</label>
              <input
                type="date"
                value={deadline}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Observações Iniciais</label>
            <textarea
              value={notes}
      ...
      {kind === "fast" && (
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Duração</label>
              <input
                autoFocus
                value={duration}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Data do Jejum</label>
              <input
                type="date"
                value={fastDate}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Motivo do Jejum</label>
            <textarea
              value={reason}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Observações do Jejum</label>
            <textarea
              value={fastNotes}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">O que aprendeu/sentiu?</label>
            <textarea
              value={learnings}
      ...
      {kind === "gratitude" && (
        <div className="space-y-3">
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Pelo que é grato hoje?</label>
            <textarea
              autoFocus
              value={gratitudeText}
      ...
      {kind === "learning" && (
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Título do Aprendizado</label>
              <input
                autoFocus
                value={title}
                onChange={e => setTitle(e.target.value)}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Origem do Ensino</label>
              <select
                value={source}
                onChange={e => setSource(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none"
              >
                <option value="culto" className="bg-[#17152b]">Pregação de Culto</option>
                <option value="leitura bíblica" className="bg-[#17152b]">Leitura Bíblica</option>
                <option value="conversa" className="bg-[#17152b]">Conversa edificante</option>
                <option value="momento de oração" className="bg-[#17152b]">Momento de Oração</option>
                <option value="outro" className="bg-[#17152b]">Outros momentos</option>
              </select>
            </div>
          </div>
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Lições que Deus te ensinou</label>
            <textarea
              value={learningNotes}
      ...
      {kind === "favorite_verse" && (
        <div className="space-y-3">
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Referência</label>
            <input
              autoFocus
              value={favVerseRef}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Texto do Versículo</label>
            <textarea
              value={favVerseText}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Por que este versículo é importante para você?</label>
            <textarea
    ...
    switch (item.kind) {
      case "prayer":
        return {
          ...
          background: "linear-gradient(135deg, oklch(0.65 0.22 355 / 0.04), oklch(0.65 0.22 355 / 0.02))",
      ...
      case "devotional":
        return {
          ...
          background: "linear-gradient(135deg, oklch(0.65 0.18 220 / 0.04), oklch(0.65 0.18 220 / 0.02))",
      ...
      case "purpose":
        return {
          ...
          background: "linear-gradient(135deg, oklch(0.6 0.2 280 / 0.04), oklch(0.6 0.2 280 / 0.02))",
      ...
      case "fast":
        return {
          ...
          background: "linear-gradient(135deg, oklch(0.65 0.2 40 / 0.04), oklch(0.65 0.2 40 / 0.02))",
      ...
      case "gratitude":
        return {
          ...
          background: "linear-gradient(135deg, oklch(0.62 0.22 15 / 0.04), oklch(0.62 0.22 15 / 0.02))",
      ...
      case "learning":
        return {
          ...
          background: "linear-gradient(135deg, oklch(0.65 0.18 155 / 0.04), oklch(0.65 0.18 155 / 0.02))",
      ...
      case "favorite_verse":
        return {
          ...
          background: "linear-gradient(135deg, oklch(0.75 0.18 80 / 0.04), oklch(0.75 0.18 80 / 0.02))",
        ...
        <button className="flex-1 text-left" onClick={() => setExp(e => !e)}>
          ...
          {item.answered_at && (
            <p className="text-[11px] text-emerald-400 mt-1 flex items-center gap-1">
              <Check className="w-3.5 h-3.5" /> Respondida em {new Date(item.answered_at).toLocaleDateString("pt-BR")}
            </p>
          )}
        </button>
        ...
        {exp && (
          ...
          >
            ...
            {item.kind === "prayer" && (
              <div className="space-y-2 text-xs">
                <div className="flex gap-2 flex-wrap mb-1">
                  {data.category && (
                    <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-full border uppercase tracking-wider ${style.tag}`}>
                      {data.category}
                    </span>
                  )}
                  <span className="text-[9px] text-white/30 flex items-center gap-1 font-mono uppercase bg-white/5 px-2 py-0.5 rounded-full border border-white/5">
                    <Lock className="w-2.5 h-2.5" /> {data.privacy === "private" ? "Privado" : "Público"}
                  </span>
                </div>
                {data.description && <p className="text-sm text-white/70 leading-relaxed mt-2 whitespace-pre-wrap">{data.description}</p>}
                ...
                {item.answered_at ? (
                  <div className="mt-3 p-3.5 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-2">
                    ...
                    {data.how_answered && (
                      <div>
                        <span className="text-white/40 block text-[9px] uppercase tracking-wider">Como Deus respondeu:</span>
                        <p className="text-white/80 leading-relaxed mt-0.5 whitespace-pre-wrap">{data.how_answered}</p>
                      </div>
                    )}
                    {data.testimony && (
                      <div className="border-t border-white/5 pt-1.5 mt-1">
                        <span className="text-white/40 block text-[9px] uppercase tracking-wider">Testemunho:</span>
                        <p className="text-white/80 leading-relaxed mt-0.5 italic whitespace-pre-wrap">"{data.testimony}"</p>
                      </div>
                    )}
                  </div>
                ) : (
                  onMarkAnswered && !answering && (
                    <button
                ...
                {answering && (
                  <div className="mt-3 p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-white">Registrar Resposta de Deus</p>
                    <textarea
                      value={howAns}
            ...
            {item.kind === "devotional" && (
              <div className="space-y-3 text-xs">
                <div className="flex justify-between items-center gap-2 flex-wrap">
                  <span className="font-semibold text-sky-300">📖 {data.verse || "Leitura Bíblica"}</span>
                ...
                {data.learned && (
                  <div>
                    <span className="text-white/40 font-semibold block uppercase tracking-wider text-[9px]">O que aprendi:</span>
                    <p className="text-sm text-white/70 leading-relaxed mt-0.5 whitespace-pre-wrap">{data.learned}</p>
                  </div>
                )}
                {data.application && (
                  <div>
                    <span className="text-white/40 font-semibold block uppercase tracking-wider text-[9px]">Como vou aplicar:</span>
                    <p className="text-sm text-white/70 leading-relaxed mt-0.5 whitespace-pre-wrap">{data.application}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {item.kind === "purpose" && (
              <div className="space-y-3.5 text-xs">
                <div className="flex justify-between text-[10px] text-white/40 font-mono">
                  <span>Início: {data.start_date ? new Date(data.start_date).toLocaleDateString("pt-BR") : "N/D"}</span>
                  {data.deadline && <span>Prazo: {new Date(data.deadline).toLocaleDateString("pt-BR")}</span>}
                </div>
                ...
                <div>
                  ...
                  <div className="h-2 rounded-full bg-white/5 border border-white/10 overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-violet-500 to-fuchsia-500" style={{ width: `${data.progress || 0}%` }} />
                  </div>
                </div>
                ...
                {onUpdatePurpose && (
                  <div className="flex gap-1.5 pt-1.5">
                    ...
                    <button
                      onClick={() => onUpdatePurpose(item, Math.min(100, (data.progress || 0) + 10))}
                      className="text-[9px] bg-white/5 hover:bg-white/10 px-2 py-1 rounded border border-white/5 text-white/60 hover:text-white transition-all"
                    >
                      +10%
                    </button>
                ...
                {data.objective && (
                  <div className="border-t border-white/5 pt-2">
                    <span className="text-white/40 font-semibold block uppercase tracking-wider text-[9px]">Objetivo:</span>
                    <p className="text-sm text-white/70 leading-relaxed mt-0.5 whitespace-pre-wrap">{data.objective}</p>
                  </div>
                )}
                {data.notes && (
                  <div>
                    <span className="text-white/40 font-semibold block uppercase tracking-wider text-[9px]">Observações:</span>
                    <p className="text-sm text-white/70 leading-relaxed mt-0.5 whitespace-pre-wrap">{data.notes}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {item.kind === "fast" && (
              <div className="space-y-3 text-xs">
                <div className="flex justify-between text-[10px] text-white/40 font-mono">
                  <span>Data: {data.date ? new Date(data.date).toLocaleDateString("pt-BR") : "N/D"}</span>
                  <span>Duração: {data.duration || "N/D"}</span>
                </div>
                {data.reason && (
                  <div>
                    <span className="text-white/40 font-semibold block uppercase tracking-wider text-[9px]">Motivo:</span>
                    <p className="text-sm text-white/70 leading-relaxed mt-0.5 whitespace-pre-wrap">{data.reason}</p>
                  </div>
                )}
                {data.learnings && (
                  <div>
                    <span className="text-white/40 font-semibold block uppercase tracking-wider text-[9px]">Aprendizados:</span>
                    <p className="text-sm text-white/70 leading-relaxed mt-0.5 whitespace-pre-wrap">{data.learnings}</p>
                  </div>
                )}
                {data.notes && (
                  <div>
            ...
            {item.kind === "learning" && (
              <div className="space-y-2 text-xs">
                ...
                {data.notes && (
                  <p className="text-sm text-white/70 leading-relaxed mt-1.5 whitespace-pre-wrap">{data.notes}</p>
                )}
              </div>
            )}
            ...
            {item.kind === "favorite_verse" && (
              <div className="space-y-3 text-xs">
                ...
                {data.reason && (
                  <div>
                    <span className="text-white/40 font-semibold block uppercase tracking-wider text-[9px]">Por que é importante para mim:</span>
                    <p className="text-sm text-white/70 leading-relaxed mt-0.5 whitespace-pre-wrap">{data.reason}</p>
                  </div>
                )}
              </div>
...
function Vida() {
  ...
  const markAnswered = async (id: string, howAnswered: string, testimony: string) => {
    ...
    if (isPublic && uid) {
      logActivity(uid, "prayer_answered", { prayerId: id, title: item.title, testimony, howAnswered });
    }

    const now = new Date().toISOString();

    if (useLocalFallback) {
      const updated = allItems.map(x => x.id === id ? { ...x, answered_at: now, body: updatedBody, updated_at: now } : x);
      setAllItems(updated);
      saveLocalItems(uid, updated);
    ...
    try {
      ...
      const updated = allItems.map(x => x.id === id ? { ...x, answered_at: now, body: updatedBody } : x);
      setAllItems(updated);
      saveLocalItems(uid, updated);
      audio.play("achievement");
  ...
  const updatePurposeProgress = async (item: Prayer, newProg: number) => {
    ...
    if (useLocalFallback) {
      const updated = allItems.map(x => x.id === item.id ? { ...x, body: updatedBody, updated_at: now } : x);
      setAllItems(updated);
      saveLocalItems(uid, updated);
  ...
  const getGroupedTimeline = () => {
    ...
    const sorted = [...allItems].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  ...
  return (
    <AppShell>
      ...
      <div className="animate-fade-up mb-6 text-left">
        ...
        <p className="text-white/40 mt-2 text-sm max-w-sm">Um diário espiritual privado para registrar a fidelidade de Deus e seu crescimento cristão.</p>
      </div>
      ...
      <div className="flex gap-2 flex-wrap mb-6 max-h-[140px] sm:max-h-none overflow-y-auto pr-1">
        {SECTIONS.map(s => {
          ...
          if (active) {
            if (s.kind === "prayer") colorStyle = { tagBg: "oklch(0.65 0.22 355 / 0.12)", tagBorder: "oklch(0.65 0.22 355 / 0.25)", tagGlow: "oklch(0.65 0.22 355 / 0.2)", tagColor: "text-rose-300" };
            else if (s.kind === "devotional") colorStyle = { tagBg: "oklch(0.65 0.18 220 / 0.12)", tagBorder: "oklch(0.65 0.18 220 / 0.25)", tagGlow: "oklch(0.65 0.18 220 / 0.2)", tagColor: "text-sky-300" };
            else if (s.kind === "purpose") colorStyle = { tagBg: "oklch(0.6 0.2 280 / 0.12)", tagBorder: "oklch(0.6 0.2 280 / 0.25)", tagGlow: "oklch(0.6 0.2 280 / 0.2)", tagColor: "text-violet-300" };
            else if (s.kind === "fast") colorStyle = { tagBg: "oklch(0.65 0.2 40 / 0.12)", tagBorder: "oklch(0.65 0.2 40 / 0.25)", tagGlow: "oklch(0.65 0.2 40 / 0.2)", tagColor: "text-orange-300" };
            else if (s.kind === "gratitude") colorStyle = { tagBg: "oklch(0.62 0.22 15 / 0.12)", tagBorder: "oklch(0.62 0.22 15 / 0.25)", tagGlow: "oklch(0.62 0.22 15 / 0.2)", tagColor: "text-red-300" };
            else if (s.kind === "learning") colorStyle = { tagBg: "oklch(0.65 0.18 155 / 0.12)", tagBorder: "oklch(0.65 0.18 155 / 0.25)", tagGlow: "oklch(0.65 0.18 155 / 0.2)", tagColor: "text-emerald-300" };
            else if (s.kind === "favorite_verse") colorStyle = { tagBg: "oklch(0.75 0.18 80 / 0.12)", tagBorder: "oklch(0.75 0.18 80 / 0.25)", tagGlow: "oklch(0.75 0.18 80 / 0.2)", tagColor: "text-amber-300" };
            else colorStyle = { tagBg: "oklch(1 0 0 / 0.1)", tagBorder: "oklch(1 0 0 / 0.15)", tagGlow: "oklch(1 0 0 / 0.05)", tagColor: "text-white" };
          }

          return (
            <button
              key={s.kind}
              onClick={() => { setTab(s.kind); setAdding(false); }}
        ...
        >
          {tab !== "timeline" ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-3 text-left">
                <div>
                  ...
                  <p className="text-[10px] text-white/30 tracking-wider">
                    {filteredItems.length} registro{filteredItems.length !== 1 ? "s" : ""} cadastrado{filteredItems.length !== 1 ? "s" : ""}
                  </p>
                </div>
                <button
              ...
              <AnimatePresence>
                {adding && (
                  <AddForm kind={tab} onAdd={add} onClose={() => setAdding(false)} />
                )}
              </AnimatePresence>
              ...
              {loading ? (
                ...
                >
                  ...
                  <p className="text-xs text-white/40 mb-5">Nenhum registro de {section.label.toLowerCase()} ainda.</p>
                  <button
            ...
            <div className="space-y-8 text-left border-l border-white/5 pl-4 sm:pl-6 ml-3 relative">
              ...
              {loading ? (
                ...
                <div className="rounded-3xl p-10 text-center border border-white/5 bg-white/1">
                  ...
                  <p className="text-xs text-white/40">Sua linha do tempo aparecerá vazia até você registrar suas primeiras orações, devocionais ou propósitos.</p>
                </div>
              ) : (
