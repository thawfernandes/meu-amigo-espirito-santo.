...
export function Stats() {
  ...
  const [activeTab, setActiveTab] = useState<"constancia" | "biblia" | "conhecimento">(
    "constancia",
  );
  ...
  const [readChaptersHistory, setReadChaptersHistory] = useState<
    { book: string; chapter: number; readAt: string }[]
  >([]);
  const [totalQuestionsCount, setTotalQuestionsCount] = useState(0);
  const [correctAnswersCount, setCorrectAnswersCount] = useState(0);
  const [quizHistory, setQuizHistory] = useState<{ date: string; isCorrect: boolean }[]>([]);
  const [unlockedBadges, setUnlockedBadges] = useState<string[]>([]);
  const [completedMonthlyChallenges, setCompletedMonthlyChallenges] = useState<
  ...
  const [highlightModalColor, setHighlightModalColor] = useState<"green" | "yellow" | "red" | null>(
    null,
  );
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: prs } = await supabase
        .from("prayers")
        .select("id, kind, title, body, answered_at, created_at")
        .eq("user_id", userId);
      ...
      const { data: nts } = await supabase
        .from("verse_notes")
        .select("book, chapter, verse, content, created_at")
        .eq("user_id", userId);
      if (nts) setNotes(nts as NoteItem[]);

      // Load Read Chapters from Supabase
      const { data: readChs } = await supabase
        .from("read_chapters")
        .select("book, chapter")
        .eq("user_id", userId);
      if (readChs) {
      ...
      if (acts) {
        // Reading history
        const readHistory = acts
          .filter((a) => a.kind === "reading")
          .map((a) => ({
            book: a.payload.book || "",
      ...
      const { data: mc } = await supabase
        ...
        .select("challenge_id, completed_at, experience")
        .eq("user_id", userId);
      if (mc) {
        setCompletedMonthlyChallenges(
          mc.map((r) => ({
            id: r.challenge_id,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const computedPercent = Math.min(
        100,
        Math.round(uniqueRead * 1.5 + totalPrayers * 4 + totalStudies * 6 + totalChallenges * 8),
      );
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const readingCount = allActivities.filter(
        (a) => a.date.startsWith(mStr) && a.type === "reading",
      ).length;
      const studiesCount = allActivities.filter(
        (a) => a.date.startsWith(mStr) && (a.type === "study" || a.type === "learning"),
      ).length;
      const prayersCount = allActivities.filter(
        (a) => a.date.startsWith(mStr) && a.type === "prayer",
      ).length;
      const challengesCount = allActivities.filter(
        (a) => a.date.startsWith(mStr) && a.type === "challenge",
      ).length;

      // Calculate unique active days in this month
      const uniqueDays = new Set(
        allActivities.filter((a) => a.date.startsWith(mStr)).map((a) => a.date),
      );
  ...
  const curiosities = useMemo(() => {
    ...
    const daysSinceStart =
      Math.ceil(Math.abs(new Date().getTime() - createdDate.getTime()) / (1000 * 60 * 60 * 24)) ||
      1;
    ...
    readChaptersHistory.forEach((ch) => {
      bookCounts[ch.book] = (bookCounts[ch.book] || 0) + 1;
    });
    highlights.forEach((h) => {
    ...
    readChaptersHistory.forEach((ch) => {
      hourCounts[new Date(ch.readAt).getHours()]++;
    });
    highlights.forEach((h) => {
      hourCounts[new Date(h.created_at).getHours()]++;
    });
    notes.forEach((n) => {
      hourCounts[new Date(n.created_at).getHours()]++;
    });
    prayers.forEach((p) => {
      hourCounts[new Date(p.created_at).getHours()]++;
    });
    ...
    if (currentStreak > 0) {
      facts.push(
        `Você está em uma sequência de constância de ${currentStreak} dia${currentStreak > 1 ? "s" : ""}.`,
      );
    } else {
    ...
    const bookWithMostHighlights = highlights.reduce(
      (acc, h) => {
    ...
    const chapterWithMostNotes = notes.reduce(
      (acc, n) => {
  ...
  const timelineMilestones = useMemo(() => {
    ...
    // Genesis completion
    const genesisChapters = readChaptersHistory
      .filter((ch) => ch.book.toLowerCase() === "gênesis")
      .map((ch) => ch.chapter);
    const uniqueGenCh = new Set(genesisChapters);
    if (uniqueGenCh.size === 50) {
      const finishDate = readChaptersHistory
        .filter((ch) => ch.book.toLowerCase() === "gênesis")
        .sort((a, b) => b.readAt.localeCompare(a.readAt))[0]?.readAt;
      milestones.push({
        icon: "📖",
    ...
    // First study
    const completedStudies = [...notebooks]
    ...
    if (completedMonthlyChallenges.length > 0) {
      const oldestChallenge = [...completedMonthlyChallenges].sort((a, b) =>
  ...
  const createdDate = profile?.created_at ? new Date(profile.created_at) : new Date();
  const diffDays =
    Math.ceil(Math.abs(new Date().getTime() - createdDate.getTime()) / (1000 * 60 * 60 * 24)) || 1;
    ...
    <AppShell>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 mb-8">
        {[
          {
            icon: Calendar,
            label: "Dias caminhando",
            v: `${diffDays} d`,
            color: "text-emerald-400",
            bg: "from-emerald-500/10 to-teal-500/5",
            glow: "oklch(0.65 0.18 155 / 0.12)",
          },
          {
            icon: Flame,
            label: "Dias consecutivos",
            v: `${walk?.streak_days || 0} d`,
            color: "text-orange-400",
            bg: "from-orange-500/10 to-amber-500/5",
            glow: "oklch(0.65 0.2 40 / 0.12)",
          },
          {
            icon: BookOpen,
            label: "Capítulos lidos",
            v: `${readChapters.length}`,
            color: "text-amber-400",
            bg: "from-amber-500/10 to-yellow-500/5",
            glow: "oklch(0.75 0.18 80 / 0.12)",
          },
          {
            icon: BookMarked,
            label: "Livros concluídos",
            v: `${BIBLE_BOOKS.filter((b) => readChapters.filter((c) => c.startsWith(`${b.name}-`)).length === b.chapters).length}`,
            color: "text-rose-400",
            bg: "from-rose-500/10 to-pink-500/5",
            glow: "oklch(0.65 0.22 355 / 0.12)",
          },
          {
            icon: Bookmark,
          ...
          },
          {
            ...
            bg: "from-fuchsia-500/10 to-indigo-500/5",
            glow: "oklch(0.6 0.2 280 / 0.12)",
          },
          {
            icon: HelpCircle,
            label: "Quizzes resolvidos",
            v: `${totalQuestionsCount}`,
            color: "text-cyan-400",
          ...
          },
          {
            ...
            v: `${completedMonthlyChallenges.length}`,
          ...
          },
          {
            ...
            glow: "oklch(0.62 0.22 15 / 0.12)",
          },
          {
            icon: CheckCircle2,
            label: "Orações respondidas",
            v: `${totalPrayersAnswered}`,
            color: "text-emerald-400",
          ...
          {
            icon: Sparkles,
            label: "Aproveitamento Quiz",
            v: `${quizPerf.pct}%`,
            color: "text-amber-300",
            bg: "from-amber-400/10 to-yellow-500/5",
            glow: "oklch(0.75 0.18 80 / 0.12)",
          },
        ].map((s, i) => (
          <div
          ...
          >
            <div className="flex items-center justify-between">
              <span
                className={`text-[10px] uppercase font-bold tracking-wider text-white/50 leading-tight`}
              >
                {s.label}
              </span>
              <s.icon className={`w-4.5 h-4.5 ${s.color} opacity-85 shrink-0`} />
            </div>
            <p className="font-display text-2xl sm:text-3xl font-bold text-white mt-3 leading-none">
              {s.v}
            </p>
          </div>
        ))}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(135deg, oklch(0.14 0.03 260 / 0.95), oklch(0.1 0.02 260 / 0.98))",
              ...
              >
                <div className="mb-4">
                  ...
                  <p className="text-xs text-white/40 mt-1">
                    Acompanhe a constância de suas atividades espirituais nos últimos meses.
                  </p>
                </div>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-white/40 mt-1">
                    Sua presença nas Escrituras e orações ao longo dos últimos meses.
                  </p>
                </div>
                      ...
                      <div key={rowIdx} className="flex gap-1 items-center">
                        ...
                        {row.map((cell, cellIdx) => {
                          ...
                          if (cell.activitiesCount > 0) {
                            if (cell.activitiesCount <= 2)
                              colorClass = "bg-emerald-500/25 border-emerald-400/20";
                            else if (cell.activitiesCount <= 4)
                              colorClass = "bg-emerald-500/50 border-emerald-400/40";
                            else
                              colorClass =
              ...
              {selectedDayActivities && (
                ...
                >
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, oklch(0.16 0.03 260 / 0.95), oklch(0.12 0.02 260 / 0.98))",
                  ...
                  >
                    ...
                    {selectedDayActivities.items.length === 0 ? (
                      <p className="text-xs text-white/45 mt-3">
                        Nenhum registro de atividade inserido neste dia específico.
                      </p>
                    ) : (
                ...
                {[
                  {
                    ...
                    dot: "bg-emerald-400",
                    label: "Coração",
                    count: highlights.filter((h) => h.color === "green").length,
                    desc: "Textos que tocaram profundamente o coração.",
                  ...
                  },
                  {
                    ...
                    dot: "bg-amber-400",
                    label: "Para Revisar",
                    count: highlights.filter((h) => h.color === "yellow").length,
                    desc: "Passagens para reler com calma.",
                  ...
                  },
                  {
                    ...
                    count: highlights.filter((h) => h.color === "red").length,
                    desc: "Versículos com dúvidas exegéticas a estudar.",
            ...
            <section className="text-left">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5 border-b border-white/5 pb-4">
                <div>
                  ...
                  <p className="text-xs text-white/40 mt-1">
                    Monitore quais livros você está lendo ou já concluiu.
                  </p>
                </div>
                ...
                <div className="flex gap-2 items-center w-full sm:w-auto">
                  ...
                  >
                    <option value="all" className="bg-[#17152b]">
                      Todos
                    </option>
                    <option value="AT" className="bg-[#17152b]">
                      A. Testamento
                    </option>
                    <option value="NT" className="bg-[#17152b]">
                      N. Testamento
                    </option>
                  </select>
                </div>
                ...
                {filteredBibleBooks.map((b) => (
                  <div
                    ...
                    style={{
                      ...
                      border:
                        b.status === "completed"
                          ? "1px solid oklch(0.65 0.18 155 / 0.4)"
                          : b.status === "in_progress"
                            ? "1px solid oklch(0.65 0.18 220 / 0.4)"
                            : "1px solid oklch(1 0 0 / 0.06)",
                    }}
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        <h4 className="font-semibold text-xs text-white/95 leading-tight">
                          {b.name}
                        </h4>
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between items-center text-[10px] text-white/40 mb-1.5 font-mono">
                        <span>
                          {b.readCount} de {b.chapters} capítulos
                        </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-white/40 mt-1">
                    Evolução do aprendizado nos questionários bíblicos.
                  </p>
                </div>

                <div className="my-5 space-y-4">
                  <div>
                    <span className="text-[10px] text-white/45 uppercase tracking-wider block font-semibold mb-1.5">
                      Tópicos com Melhor Aproveitamento
                    </span>
                    <ul className="space-y-1.5">
                      {quizPerf.best.map((topic, i) => (
                        <li
                          key={i}
                          className="text-xs text-emerald-300 flex items-center gap-1.5 font-medium"
                        >
                  ...
                  <div className="border-t border-white/5 pt-3.5">
                    <span className="text-[10px] text-white/45 uppercase tracking-wider block font-semibold mb-1.5">
                      Tópicos para Revisão e Reforço
                    </span>
                    <ul className="space-y-1.5">
                      {quizPerf.rev.map((topic, i) => (
                        <li
                          key={i}
                          className="text-xs text-amber-300/80 flex items-center gap-1.5 font-medium"
                        >
                ...
                <div className="text-[10px] text-white/30 border-t border-white/5 pt-3.5">
                  * Os dados de conhecimento bíblico visam puramente inspirar constância no
                  aprendizado diário.
                </div>
              </div>
              ...
              >
                <div className="mb-5">
                  ...
                  <p className="text-xs text-white/40 mt-1">
                    Momentos marcantes da sua constância com Deus.
                  </p>
                </div>
                ...
                <div className="relative border-l border-white/10 ml-3 pl-5 space-y-5 max-h-[250px] overflow-y-auto pr-1">
                  {timelineMilestones.length === 0 ? (
                    <div className="text-xs text-white/40 italic py-4">
                      Sua linha do tempo está aguardando os primeiros marcos! Comece lendo a Bíblia
                    ...
                    timelineMilestones.map((ms, idx) => (
                      <div key={idx} className="relative">
                        {/* circle */}
                        <span className="absolute -left-[27px] top-1 w-3.5 h-3.5 rounded-full bg-sky-400 flex items-center justify-center text-[10px] ring-4 ring-slate-900"></span>
            ...
            <section className="text-left">
              <div className="border-b border-white/5 pb-3 mb-6">
                ...
                <p className="text-xs text-white/45 mt-1">
                  Conquistas celebrando a leitura constante e a fé prática.
                </p>
              </div>
                ...
                {[
                  {
                    ...
                    desc: "Respondeu à primeira questão bíblica.",
                    icon: "📖",
                    check: correctAnswersCount >= 1,
                  },
                  {
                    id: "constancy",
                    title: "Constância",
                    desc: "Respondeu a 10 questões bíblicas.",
                    icon: "🌱",
                    check: correctAnswersCount >= 10,
                  },
                  {
                    id: "notebook_creator",
                    title: "Escrivão",
                    desc: "Criou o primeiro caderno de estudos.",
                    icon: "📚",
                    check: notebooks.length >= 1,
                  },
                  {
                    id: "prayer_life",
                    title: "Vida de Clamor",
                    desc: "Registrou sua primeira oração.",
                    icon: "🙏",
                    check: totalPrayersCount >= 1,
                  },
                  {
                    id: "charity_first",
                    title: "Fé com Obras",
                    desc: "Concluiu o primeiro Desafio Mensal.",
                    icon: "❤️",
                    check: completedMonthlyChallenges.length >= 1,
                  },
                ].map((badge) => (
                  <div
                    key={badge.id}
                    ...
                    style={{
                      background: badge.check
                        ...
                        : "oklch(1 0 0 / 0.03)",
                      border: badge.check
                        ? "1px solid oklch(0.65 0.18 255 / 0.3)"
                        : "1px solid oklch(1 0 0 / 0.06)",
                    }}
                  >
                    <span className="text-3.5xl mb-2.5 block">{badge.icon}</span>
                    <h4 className="text-[11px] font-semibold text-white/95 leading-tight">
                      {badge.title}
                    </h4>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-white/50 leading-relaxed mt-1 max-w-lg">
              Esta tela funciona como um diário visual de comunhão. Cada capítulo lido, oração
              anotada e quiz respondido são pequenos passos que constroem uma grande história de fé
              ao longo do tempo. O importante é continuar avançando, um dia de cada vez.
            </p>
          </div>
        </div>
        <button
            ...
            >
              <div className="flex items-center justify-between border-b border-white/5 pb-3.5 mb-4 shrink-0">
                <h3 className="font-display text-md text-white font-semibold flex items-center gap-2">
                  <Bookmark className="w-5 h-5 text-violet-400" />
                  Versículos Marcados —{" "}
                  {highlightModalColor === "green"
                    ? "Coração"
                    : highlightModalColor === "yellow"
                      ? "Revisar Depois"
                      : "Estudo Profundo"}
                </h3>
                <button
