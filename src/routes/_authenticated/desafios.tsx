...
import {
  Target, Calendar, CheckCircle2, Flame, Loader2, Trophy,
  Award, HelpCircle, BookOpen, ChevronLeft, ChevronRight, X,
  Sparkles, RefreshCw, AlertCircle, Play
} from "lucide-react";
import { toast } from "sonner";
import { useAudio } from "@/components/audio/AudioProvider";
import { logActivity } from "@/lib/activity";
import { getAdaptiveQuiz, BibleQuestion, recordAnswer, recordQuizCompletion } from "@/lib/quiz-data";
import { getActiveChallengesForMonth, getAllChallenges, MonthlyChallenge } from "@/lib/monthly-challenges-data";
...
function MonthlyChallengeCard({
  ...
  return (
    <motion.div
      ...
      style={{
        background: "linear-gradient(135deg, oklch(0.18 0.04 260 / 0.85), oklch(0.14 0.03 260 / 0.95))",
    ...
    >
      ...
      {/* Title & Description */}
      <div>
        <h3 className="font-display text-base sm:text-lg font-semibold text-white leading-snug">{challenge.title}</h3>
      ...
      <div className="text-xs">
        <span className="text-white/40 block text-[9px] uppercase tracking-wider font-semibold">Objetivo:</span>
        <p className="text-white/80 font-medium leading-relaxed mt-0.5">{challenge.objective}</p>
      </div>

      {/* Practical Suggestions */}
      <div className="text-xs">
        <span className="text-white/40 block text-[9px] uppercase tracking-wider font-semibold mb-1">Sugestões Práticas:</span>
        ...
        <div className="mt-2 p-3.5 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-1.5 text-xs text-left">
          ...
          {completedInfo.experience && (
            <div className="border-t border-white/5 pt-1.5 mt-1.5">
              <span className="text-white/40 block text-[9px] uppercase tracking-wider mb-0.5">Relato da Experiência:</span>
          ...
          {!showConfirm ? (
            ...
            >
              ...
              <div className="flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => { setShowConfirm(false); setExperience(""); }}
...
function Challenges() {
  ...
  useEffect(() => {
    if (!dailyQuestions || dailyQuestions.length === 0 || dailyStep >= dailyQuestions.length) return;
  ...
  const handleDailySubmit = () => {
    ...
    if (q.type === "chronological") {
      ...
      correct = q.matchingLeft?.every((leftItem, idx) => {
  ...
  const handleReviewSubmit = () => {
    ...
    if (q.type === "chronological") {
      ...
      correct = (reviewSelectedAns || "").toLowerCase().trim() === q.correctAnswer.toLowerCase().trim();
    }
          ...
          {loading ? (
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              ...
              >
                ...
                <div>
                  <h4 className="font-display text-sm font-semibold text-white">Sobre os Desafios Mensais</h4>
                  <p className="text-xs text-white/50 leading-relaxed mt-1">
                    Esta seção nos lembra que o Evangelho também é vivido através de atitudes diárias de amor ao próximo, 
                    serviço, compaixão e misericórdia. Todos os meses, disponibilizamos novos desafios práticos baseados nas Escrituras para te ajudar a colocar a fé em ação. Eles são privados e voluntários.
                  </p>
                </div>
              </div>
              ...
              <section className="text-left">
                <p className="text-[10px] uppercase tracking-[0.3em] text-rose-400/60 mb-3.5 flex items-center gap-1.5 font-semibold">
                  <Flame className="w-3 h-3 text-rose-400 animate-pulse" /> Ativos em {capitalizedMonth} de {new Date().getFullYear()}
                </p>
                ...
                <div className="mt-8 border-t border-white/5 pt-6 text-left">
                  ...
                  >
                    <span className="text-xs font-semibold text-white/70 flex items-center gap-2">
                      ...
                      <span>📜 Histórico de Desafios Concluídos ({completedChallenges.length})</span>
                    </span>
        ...
        <div className="space-y-8 animate-in fade-in duration-300">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-white/40 uppercase tracking-wider">Seu Perfil de Estudo</p>
            ...
            <div className="flex gap-3 flex-wrap justify-center sm:justify-end">
              <div className="rounded-xl px-4 py-2 bg-white/3 border border-white/5 text-center min-w-[80px]">
                <p className="text-lg font-bold text-emerald-400 font-mono leading-none">{statsCorrect}</p>
                <p className="text-[9px] text-white/40 uppercase tracking-wider mt-1.5">Acertos</p>
              </div>
              <div className="rounded-xl px-4 py-2 bg-white/3 border border-white/5 text-center min-w-[80px]">
                <p className="text-lg font-bold text-amber-300 font-mono leading-none">{reviewList.length}</p>
                <p className="text-[9px] text-white/40 uppercase tracking-wider mt-1.5">Revisar</p>
              </div>
              <div className="rounded-xl px-4 py-2 bg-white/3 border border-white/5 text-center min-w-[80px]">
                <p className="text-lg font-bold text-violet-300 font-mono leading-none">{unlockedBadges.length} / 4</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div
              ...
              style={{
                background: "linear-gradient(145deg, oklch(0.65 0.18 255 / 0.12), oklch(0.58 0.2 270 / 0.06))",
            ...
            >
              <div className="text-left">
                ...
                <h3 className="font-display text-lg text-white font-semibold mb-2">Desafio Diário Adaptativo</h3>
                <p className="text-xs text-white/50 leading-relaxed mb-6">
                  Responda a um quiz dinâmico de 5 questões escolhidas conforme sua maturidade de leitura. Excelente para fixar a Palavra de Deus em sua mente.
                </p>
              </div>
              <button
            ...
            <div
              className="rounded-3xl p-6 sm:p-7 flex flex-col justify-between"
              style={{
                background: "linear-gradient(145deg, oklch(0.65 0.18 80 / 0.12), oklch(0.60 0.2 65 / 0.06))",
            ...
            >
              <div className="w-full text-left">
                ...
                <h3 className="font-display text-lg text-white font-semibold mb-2">Revisão Inteligente</h3>
                <p className="text-xs text-white/50 leading-relaxed mb-4">
                  Estude novamente as passagens ou perguntas que geraram dúvidas ou erros em quizzes anteriores para retirá-las da fila.
                </p>
          ...
          <section className="mt-12 text-left">
            <div className="border-b border-white/5 pb-3 mb-6">
              ...
              <p className="text-xs text-white/40 mt-1">Conquiste medalhas estudando a Bíblia e acertando quizzes.</p>
            </div>
                  ...
                  <div
                    ...
                    style={{
                      background: isUnlocked
                        ...
                        : "oklch(1 0 0 / 0.03)",
                      border: isUnlocked ? `1px solid ${badge.border}` : "1px solid oklch(1 0 0 / 0.06)",
                      boxShadow: isUnlocked ? `0 12px 32px ${badge.glow}` : "none",
                    }}
                  >
                    <span className="text-4xl mb-3 block">{badge.icon}</span>
                    <h4 className="text-xs font-semibold text-white/95 leading-tight">{badge.title}</h4>
                    <p className="text-[10px] text-white/40 mt-1.5 leading-snug">{badge.description}</p>
                    {isUnlocked ? (
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-white/5 pb-3">
                ...
                <span className="text-[10px] font-mono text-white/40">
                  {dailyStep < dailyQuestions.length ? `${dailyStep + 1} de ${dailyQuestions.length}` : "Concluído"}
                </span>
              </div>
              ...
              {dailyStep < dailyQuestions.length ? (
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {hasSubmittedAns && (
                        <p className="text-xs text-white/55 mt-1 text-left">
                          Resposta correta: <span className="text-emerald-400 font-semibold">{dailyQuestions[dailyStep].correctAnswer}</span>
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {dailyQuestions[dailyStep].type === "chronological" && (
                    <div className="space-y-3.5 text-left">
                      <div>
                        <p className="text-[10px] text-white/40 uppercase tracking-wider mb-2">Sua Sequência (Clique para remover):</p>
                        <div className="min-h-[44px] p-2.5 rounded-xl bg-white/3 border border-white/5 flex flex-wrap gap-1.5 items-center">
                          {selectedChronological.length === 0 ? (
                            <span className="text-[11px] text-white/20 italic">Selecione os itens abaixo na ordem correta...</span>
                          ) : (
                            selectedChronological.map((item, index) => (
                              <button
                                ...
                                disabled={hasSubmittedAns}
                                onClick={() => {
                                  setSelectedChronological(selectedChronological.filter(x => x !== item));
                              ...
                              >
                                <span className="text-[8px] bg-violet-500/30 text-white rounded-full w-3.5 h-3.5 flex items-center justify-center shrink-0">{index + 1}</span>
                                {item}
                              </button>
                            ))
                          )}
                      ...
                      {!hasSubmittedAns && shuffledChronological.length > 0 && (
                        <div>
                          <p className="text-[10px] text-white/40 uppercase tracking-wider mb-2">Opções Disponíveis:</p>
                            ...
                            {shuffledChronological.map(item => (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setShuffledChronological(shuffledChronological.filter(x => x !== item));
                                }}
                      ...
                      {!hasSubmittedAns && selectedChronological.length > 0 && (
                        <button
                          onClick={() => {
                            setShuffledChronological([...dailyQuestions[dailyStep].chronologicalItems!].sort(() => Math.random() - 0.5));
                            setSelectedChronological([]);
                          }}
                      ...
                      {hasSubmittedAns && (
                        <div className="p-3.5 rounded-xl bg-white/2 border border-white/5 space-y-1.5">
                          <p className="text-[10px] uppercase font-bold text-white/40">Gabarito Cronológico:</p>
                  ...
                  {dailyQuestions[dailyStep].type === "matching" && (
                    <div className="space-y-4 text-left">
                      <p className="text-[10px] text-white/40 uppercase tracking-wider">Relacione cada item:</p>
                      
                      <div className="space-y-3">
                        {dailyQuestions[dailyStep].matchingLeft?.map((leftItem, idx) => (
                          <div key={leftItem} className="space-y-1 bg-white/2 border border-white/5 p-3 rounded-2xl">
                            <span className="text-xs font-semibold text-white/90">{leftItem}</span>
                            
                            <select
                              disabled={hasSubmittedAns}
                              value={matchingSelections[leftItem] || ""}
                              onChange={(e) => setMatchingSelections({
                                ...matchingSelections,
                            ...
                            >
                              <option value="" className="bg-[#141225] text-white/40">Selecione correspondência...</option>
                              {shuffledMatchingRight.map(opt => (
                                <option key={opt} value={opt} className="bg-[#141225] text-white">{opt}</option>
                              ))}
                            </select>

                            {hasSubmittedAns && (
                              <p className="text-[10px] text-white/50 mt-1 leading-normal">
                                Resposta correta: <span className="text-emerald-400 font-semibold">{dailyQuestions[dailyStep].matchingRight?.[idx]}</span>
                              </p>
                            )}
                          </div>
                        ))}
                  ...
                  {hasSubmittedAns && (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-white/40 font-mono">Leitura sugerida: {dailyQuestions[dailyStep].suggestedReading}</p>
                    </motion.div>
                  )}
                  ...
                  <div className="flex justify-end gap-2 pt-2 border-t border-white/5">
                    {!hasSubmittedAns ? (
                      <button
                        ...
                        disabled={
                          (dailyQuestions[dailyStep].type === "chronological" && selectedChronological.length < dailyQuestions[dailyStep].chronologicalItems!.length) ||
                          (dailyQuestions[dailyStep].type === "matching" && Object.keys(matchingSelections).length < dailyQuestions[dailyStep].matchingLeft!.length) ||
                          ((dailyQuestions[dailyStep].type === "multiple-choice" || dailyQuestions[dailyStep].type === "true-false" || dailyQuestions[dailyStep].type === "context" || dailyQuestions[dailyStep].type === "fill-blank") && !selectedAns)
                        }
                      ...
                      <button
                        ...
                        style={{ background: "linear-gradient(135deg, oklch(0.58 0.2 280), oklch(0.50 0.22 300))" }}
                      >
                        {dailyStep === dailyQuestions.length - 1 ? "Ver Resultados" : "Próxima Pergunta"}
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4 text-center py-4">
                  ...
                  <h4 className="font-display text-lg text-white font-semibold">Quiz do Dia Concluído!</h4>
                  <p className="text-xs text-white/50">
                    Você acertou <span className="text-violet-300 font-bold">{dailyScore}</span> de{" "}
                    <span className="text-white font-bold">{dailyQuestions.length}</span> perguntas adaptadas.
                  </p>
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-white/5 pb-3">
                <h3 className="font-display text-md text-white font-medium flex items-center gap-2">
                  ...
                  Revisar: {reviewQuestion.book ? `${reviewQuestion.book} ${reviewQuestion.chapter}` : "Estudo"}
                </h3>
                <button
                  onClick={() => setReviewQuestion(null)}
              ...
              <div className="space-y-4">
                ...
                {reviewQuestion.type === "fill-blank" && (
                  <div className="space-y-2">
                    ...
                    {reviewHasSubmitted && (
                      <p className="text-xs text-white/55 mt-1 text-left">
                        Resposta correta: <span className="text-emerald-400 font-semibold">{reviewQuestion.correctAnswer}</span>
                      </p>
                    )}
                  </div>
                )}
                ...
                {reviewQuestion.type === "chronological" && (
                  <div className="space-y-3.5 text-left">
                        ...
                        {revSelectedChronological.length === 0 ? (
                          ...
                          revSelectedChronological.map((item, index) => (
                            <button
                              ...
                              disabled={reviewHasSubmitted}
                              onClick={() => {
                                setRevSelectedChronological(revSelectedChronological.filter(x => x !== item));
                          ...
                          {revShuffledChronological.map(item => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setRevShuffledChronological(revShuffledChronological.filter(x => x !== item));
                              }}
                              className="w-full text-left rounded-xl bg-white/5 border border-white/8 text-white/80 hover:bg-white/8 hover:text-white px-4 py-2.5 text-xs transition-all active:scale-95"
                            >
                              {item}
                    ...
                    {!reviewHasSubmitted && revSelectedChronological.length > 0 && (
                      <button
                        onClick={() => {
                          setRevShuffledChronological([...reviewQuestion.chronologicalItems!].sort(() => Math.random() - 0.5));
                          setRevSelectedChronological([]);
                        }}
                        className="text-[10px] font-semibold text-white/40 hover:text-white/60 transition-colors flex items-center gap-1"
                      >
                      ...
                      {reviewQuestion.matchingLeft?.map((leftItem, idx) => (
                        <div key={leftItem} className="space-y-1 bg-white/2 border border-white/5 p-3 rounded-2xl">
                          ...
                          <select
                            ...
                            onChange={(e) => setRevMatchingSelections({
                          ...
                          {reviewHasSubmitted && (
                            <p className="text-[10px] text-white/50 mt-1 leading-normal">
                              Resposta correta: <span className="text-emerald-400 font-semibold">{reviewQuestion.matchingRight?.[idx]}</span>
                            </p>
                          )}
                        </div>
                      ))}
                ...
                {reviewHasSubmitted && (
                  ...
                  >
                    <p className="font-semibold text-white">
                      {reviewIsCorrect ? "🎉 Parabéns! Resposta Correta!" : "❌ Resposta Incorreta!"}
                    </p>
                    <p className="leading-relaxed">{reviewQuestion.explanation}</p>
                    <p className="text-[10px] text-white/40 font-mono">Leitura sugerida: {reviewQuestion.suggestedReading}</p>
                  </motion.div>
                )}
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-white/5">
                  {!reviewHasSubmitted ? (
                    <button
                      ...
                      disabled={
                        (reviewQuestion.type === "chronological" && revSelectedChronological.length < reviewQuestion.chronologicalItems!.length) ||
                        (reviewQuestion.type === "matching" && Object.keys(revMatchingSelections).length < reviewQuestion.matchingLeft!.length) ||
                        ((reviewQuestion.type === "multiple-choice" || reviewQuestion.type === "true-false" || reviewQuestion.type === "context" || reviewQuestion.type === "fill-blank") && !reviewSelectedAns)
                      }
                      className="rounded-xl px-4 py-2 text-xs font-semibold text-white hover:brightness-110 disabled:opacity-40 transition-all active:scale-95"
                      style={{ background: "linear-gradient(135deg, oklch(0.58 0.2 280), oklch(0.50 0.22 300))" }}
                    >
          ...
          <>
            ...
            >
              <motion.div
                ...
                style={{
                  background: "linear-gradient(135deg, oklch(0.2 0.04 260 / 0.95), oklch(0.14 0.03 260 / 0.98))",
              ...
              >
                ...
                <div className="space-y-2">
                  <h3 className="font-display text-xl font-bold text-white">Desafio Concluído! 🌟</h3>
                  <p className="text-xs text-white/50 uppercase tracking-widest font-mono">Fé em Ação</p>
                </div>

                <div className="p-4 rounded-2xl bg-white/5 border border-white/5 space-y-1">
                  <span className="text-2xl block">{celebratingChallenge.emoji}</span>
                  <p className="text-xs font-semibold text-white/90 leading-snug">{celebratingChallenge.title}</p>
                </div>

                <p className="text-xs text-white/60 leading-relaxed px-2">
                  Você colocou o Evangelho em prática hoje. Lembre-se: a fé sem obras é infrutífera, mas o amor demonstrado em atitudes aproxima as pessoas do amor do Pai.
                </p>
