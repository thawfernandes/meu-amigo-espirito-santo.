...
function highlightBibleRefs(
  text: string,
  onClickRef: (book: string, ch: number, vs?: number) => void,
): React.ReactNode[] {
...
function extractBibleRefs(
  content: any,
): { full: string; book: string; chapter: number; verse?: number }[] {
  if (!content) return [];
  let text = "";
...
export function EstudosPage() {
  ...
  const [markedVerses, setMarkedVerses] = useState<
    { book: string; chapter: number; verse: number; color: string }[]
  >([]);
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder:
          'Escreva aqui suas observações, insights, análises... Use os botões acima para formatar o texto. Referências bíblicas como "João 3:16" são detectadas automaticamente.',
      }),
  ...
  const handleToggleFavorite = async (nb: Notebook) => {
    ...
    setNotebooks((prev) =>
      prev.map((n) => (n.id === nb.id ? { ...n, is_favorite: !n.is_favorite } : n)),
    );
    if (selectedNotebook?.id === nb.id)
      setSelectedNotebook((prev) => (prev ? { ...prev, is_favorite: !prev.is_favorite } : null));
  };
  ...
  const handleAutoSavePage = useCallback(
    async (content: any) => {
  ...
  const handleAddDoubt = async () => {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      toast.error("Erro ao registrar dúvida.");
      return;
    }
  ...
  return (
    <AppShell>
      ...
      <div className="animate-fade-up mb-6">
        <p className="text-[10px] uppercase tracking-[0.3em] text-violet-400/60 mb-1">
          Hub de Estudos
        </p>
        ...
        <p className="text-white/40 mt-1.5 text-sm max-w-md">
          Cadernos sincronizados entre dispositivos. Crie páginas ricas com editor completo e
          integração bíblica.
        </p>
      </div>
            ...
            <button
              ...
              style={
                isActive
                  ? {
                      background:
                        "linear-gradient(135deg, oklch(0.65 0.18 255 / 0.25), oklch(0.58 0.2 280 / 0.15))",
            ...
            {!selectedNotebook ? (
              <>
                ...
                <div className="flex justify-between items-center gap-3">
                  <h2 className="font-display text-lg text-white font-medium text-left">
                    Seus Cadernos de Estudo
                  </h2>
                  <button
                ...
                {loading ? (
                  ...
                  <div className="text-center py-16 space-y-3">
                    ...
                    <p className="text-white/30 text-xs">
                      Crie seu primeiro caderno de estudo bíblico!
                    </p>
                  </div>
                ) : (
                  /* Notebooks Grid */
                    ...
                    {notebooks.map((note) => (
                      ...
                      >
                        <div>
                          ...
                          <h3 className="font-display text-md text-white font-semibold leading-snug group-hover:text-violet-300 transition-colors mt-2">
                            {note.title}
                          </h3>
                        ...
                        <div className="mt-5 border-t border-white/5 pt-3 flex items-center justify-between text-[10px] text-white/30 font-mono">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-medium ${note.status === "completed" ? "text-emerald-400 bg-emerald-400/10" : "text-violet-300 bg-violet-400/10"}`}
                          >
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <h2 className="font-display text-2xl text-white font-semibold">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, oklch(0.58 0.2 280 / 0.3), oklch(0.50 0.22 300 / 0.2))",
                        border: "1px solid oklch(0.58 0.2 280 / 0.4)",
                      }}
                    >
                  ...
                  {loadingPages ? (
                    ...
                    >
                      ...
                      <p className="text-white/25 text-xs mt-1">
                        Crie a primeira página deste caderno.
                      </p>
                    </div>
                  ) : (
                    pages.map((page, idx) => (
              ...
              >
                ...
                {editor && (
                  <div
                    className="flex flex-wrap items-center gap-1 p-2 rounded-xl"
                    style={{
                      background: "oklch(1 0 0 / 0.04)",
                      border: "1px solid oklch(1 0 0 / 0.08)",
                    }}
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(160deg, oklch(0.16 0.03 260 / 0.95), oklch(0.12 0.02 260 / 0.98))",
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button
                    onClick={() => setShowNewNotebookModal(false)}
                    className="p-1.5 rounded-full hover:bg-white/10 text-white/30"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="mt-5 flex justify-end gap-2">
                  <button
                    onClick={() => setShowNewNotebookModal(false)}
                    className="rounded-xl px-4 py-2 text-sm text-white/50 hover:text-white hover:bg-white/10 transition-all"
                  >
                    Cancelar
                  </button>
                  <button
              ...
              {exegeseSubTab === "comparacao" && (
                <div className="space-y-4">
                  <div className="p-5 rounded-2xl bg-white/5 border border-white/10 flex flex-wrap items-center justify-between gap-4 text-left">
                    <div>
                      <h3 className="font-display text-md text-white font-medium">
                        Comparação Lado a Lado
                      </h3>
                      <p className="text-xs text-white/40">
                        Selecione um versículo chave para comparar as traduções.
                      </p>
                    </div>
                    <div className="flex gap-2 items-center">
                      ...
                      >
                        <option value="João" className="bg-neutral-900 text-white">
                          João
                        </option>
                        <option value="Gênesis" className="bg-neutral-900 text-white">
                          Gênesis
                        </option>
                        <option value="Salmos" className="bg-neutral-900 text-white">
                          Salmos
                        </option>
                      </select>
                      <select
                      ...
                      >
                        ...
                        <option value={23} className="bg-neutral-900 text-white">
                          Cap. 23
                        </option>
                      </select>
                      <select
                      ...
                      >
                        ...
                        <option value={16} className="bg-neutral-900 text-white">
                          v. 16
                        </option>
                      </select>
                    </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4 text-left">
                    {Object.entries(getVerseComparison(compBook, compChapter, compVerse)).map(
                      ([trans, text]) => (
                        <div
                          key={trans}
                        ...
                        >
                          <div>
                            ...
                            <p className="text-sm text-white/85 leading-relaxed mt-2 italic">
                              "{text}"
                            </p>
                          </div>
                        </div>
                    ...
                    <div className="md:col-span-1 space-y-2">
                      {DICTIONARY.filter((d) =>
                        ...
                        <button
                          ...
                          style={
                            selectedDictTerm?.term === item.term
                              ? {
                    ...
                    <div className="md:col-span-2">
                      {selectedDictTerm ? (
                        <div className="p-6 rounded-3xl bg-white/5 border border-white/10 space-y-4 text-left">
                          <h3 className="font-display text-2xl text-amber-300">
                            {selectedDictTerm.term}
                          </h3>
                          ...
                          <div>
                            ...
                            <p className="text-xs text-white/60 font-mono mt-0.5">
                              {selectedDictTerm.whereItAppears}
                            </p>
                          </div>
                        ...
                        <div className="p-6 rounded-3xl bg-white/5 border border-white/10 space-y-4 text-left">
                          ...
                          <div>
                            <span className="text-[10px] text-white/40 uppercase tracking-wider block mb-2">
                              Acontecimentos e Linha do Tempo
                            </span>
                            ...
                            <div>
                              ...
                              <div className="flex gap-1.5 flex-wrap mt-1">
                                {selectedCharacter.mainVerses.map((v) => (
                                  <span
                                    key={v}
                                    className="text-[10px] font-mono text-white/50 bg-white/5 rounded px-2 py-0.5"
                                  >
                                    {v}
                                  </span>
                                ))}
                      ...
                      {ARCHEOLOGY.map((item) => (
                        ...
                        >
                          <div>
                            ...
                            <p className="text-[10px] text-violet-300 font-mono mt-1">
                ...
                <div className="space-y-4 text-left">
                  <div className="p-5 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-white/40">
                        Análise e interpretação exegética dos versículos, separadas por teólogos.
                      </p>
                    </div>
                  ...
                  <div className="border-t border-white/5 pt-5">
                    <h4 className="text-xs font-semibold text-white mb-2 flex items-center gap-1.5">
                      <Layers className="w-3.5 h-3.5 text-amber-400" /> Referências Cruzadas para
                      João 3:16
                    </h4>
                ...
                <div className="space-y-5 text-left">
                  <div className="p-5 rounded-3xl bg-white/3 border border-white/5 space-y-4">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4 text-xs">
                      <div>
                        <span className="text-white/40 block uppercase tracking-wider text-[9px]">
                          Autor
                        </span>
                        <span className="text-white/80 font-medium">
                          Apóstolo João (o discípulo amado), em Éfeso.
                        </span>
                      </div>
                      <div>
                        <span className="text-white/40 block uppercase tracking-wider text-[9px]">
                        ...
                        <span className="text-white/80 font-medium">
                          Cristãos judeus e gentios no Império Romano do final do século I.
                        </span>
                      </div>
                      <div>
                        <span className="text-white/40 block uppercase tracking-wider text-[9px]">
                        ...
                        <span className="text-white/80 font-medium">
                          Grande influência filosófica grega (Gnosticismo primitivo) e cultura das
                          Festas Judaicas (Páscoa, Tabernáculos).
                        </span>
                      </div>
                      <div>
                        <span className="text-white/40 block uppercase tracking-wider text-[9px]">
                        ...
                        <span className="text-white/80 font-medium">
                          Destruição do Templo de Jerusalém (70 d.C.) já ocorrida; tensões elevadas
                          entre o sinédrio judaico e os seguidores de Jesus.
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5 rounded-3xl bg-gradient-to-br from-violet-950/20 to-neutral-900 border border-violet-500/15">
                    <h4 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
                      <Lightbulb className="w-4 h-4 text-amber-300" /> Você Sabia? (Curiosidades
                      Bíblicas)
                    </h4>
                    <div className="space-y-3 text-xs text-white/60">
                      <div className="flex items-start gap-2">
                        ...
                        <p>
                          O livro de Salmos é o mais longo da Bíblia, e o Salmo 119 é o capítulo
                          mais longo.
                        </p>
                      </div>
                      <div className="flex items-start gap-2">
                        <span className="text-violet-400">•</span>
                        <p>
                          A frase 'Não temas' ou semelhantes aparece mais de 300 vezes na Bíblia,
                          servindo de encorajamento diário.
                        </p>
                      </div>
                      <div className="flex items-start gap-2">
                        <span className="text-violet-400">•</span>
                        <p>
                          O nome Jerusalém aparece mais de 800 vezes no texto bíblico canonizado.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                >
                  <div className="flex-1 space-y-1.5 text-left">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-semibold text-sm text-white leading-normal">
                        {doubt.question}
                      </h4>
                    </div>
                    {doubt.answer ? (
                      <p className="text-xs text-white/70 italic border-l-2 border-violet-500/20 pl-3 ml-1">
                        {doubt.answer}
                      </p>
                    ) : (
                      <p className="text-[11px] text-white/30 italic pl-4">
                        Ainda sem resposta de estudo.
                      </p>
                    )}
                    {doubt.references && (
                      <p className="text-[10px] text-white/30 pl-4 font-mono">
                        Refs: {doubt.references}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        await supabase
                          .from("study_doubts")
                          .update({ is_resolved: newVal })
                          .eq("id", doubt.id);
                        setDoubts((prev) =>
                          prev.map((d) => (d.id === doubt.id ? { ...d, is_resolved: newVal } : d)),
                        );
                      }}
              ...
              <div className="space-y-5">
                ...
                <div className="flex gap-1.5 overflow-x-auto pb-1.5 scrollbar-none border-b border-white/5 text-xs font-medium">
                  {[
                    ...
                    {
                      id: "commentaries",
                      label: `Comentários (${searchResults.commentaries.length})`,
                    },
                ...
                <div className="space-y-4">
                  {/* Versículos */}
                  {(searchCategory === "all" || searchCategory === "verses") &&
                    searchResults.verses.length > 0 && (
                      <div className="space-y-2">
                        <h4 className="text-xs uppercase font-mono tracking-wider text-violet-400">
                          Versículos
                        </h4>
                  ...
                  {/* Personagens */}
                  {(searchCategory === "all" || searchCategory === "characters") &&
                  ...
                  {/* Lugares / Arqueologia */}
                  {(searchCategory === "all" || searchCategory === "places") &&
                  ...
                  {/* Palavras / Dicionario */}
                  {(searchCategory === "all" || searchCategory === "words") &&
                  ...
                  {/* Comentários */}
                  {(searchCategory === "all" || searchCategory === "commentaries") &&
                    searchResults.commentaries.length > 0 && (
                      <div className="space-y-2">
                        <h4 className="text-xs uppercase font-mono tracking-wider text-violet-400">
                          Comentários de Estudiosos
                        </h4>
                  ...
                  {/* Mapas */}
                  {(searchCategory === "all" || searchCategory === "maps") &&
                  ...
                  {/* Linha do Tempo */}
                  {(searchCategory === "all" || searchCategory === "timeline") &&
        ...
        {activeTab === "marcadores" && (
          <div className="space-y-6 text-left">
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, oklch(0.60 0.18 255 / 0.1), oklch(0.50 0.2 280 / 0.05))",
              ...
              {markedCount > 0 && (
                <div className="mt-5 space-y-2">
                  <span className="text-[10px] text-white/30 uppercase tracking-wider block">
                    Versículos salvos para revisão
                  </span>
            ...
            <div>
              ...
              <p className="text-sm text-white/40 mb-4">
                Complete as tarefas para desenvolver sua autonomia no aprendizado exegético.
              </p>
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`rounded-xl p-2 border transition-all shrink-0 ${
                          isCompleted
                            ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-400 animate-in zoom-in duration-200"
              ...
              <div className="space-y-3.5 text-left">
                <div>
                  <label className="text-[10px] uppercase text-white/40 block mb-1">
                    Título do Estudo (Ex: O Evangelho de João)
                  </label>
                  <input
                    type="text"
                    value={newNoteTitle}
            ...
            >
              ...
              <div className="space-y-4 text-left">
                <div>
                  ...
                  <p className="text-sm font-semibold text-white leading-normal mt-0.5">
                ...
                <div>
                  <label className="text-[10px] uppercase text-white/40 block mb-1">
                    Sua Resposta / Conclusão de Estudo
                  </label>
                  <textarea
                    rows={4}
                ...
                <div>
                  <label className="text-[10px] uppercase text-white/40 block mb-1">
                    Referências Utilizadas (opcional)
                  </label>
                  <input
                    type="text"
                    value={doubtRefsText}
