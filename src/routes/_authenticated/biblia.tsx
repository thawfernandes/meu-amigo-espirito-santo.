...
const COLORS: Record<
  HighlightColor,
  { dot: string; bg: string; label: string; explanation: string }
> = {
  green: {
    dot: "bg-emerald-400",
    ...
    explanation:
      "Usado para versículos que o usuário compreendeu e que falaram diretamente ao seu coração. São textos que ele deseja lembrar, aplicar ou revisitar futuramente.",
  },
  yellow: {
    dot: "bg-amber-400",
    ...
    explanation:
      "Para passagens que despertaram curiosidade ou que ainda não ficaram totalmente claras. O usuário entende parte do texto, mas sente que precisa voltar e estudá-lo com mais calma.",
  },
  red: {
    dot: "bg-rose-400",
    ...
    explanation:
      "Para versículos que geraram dúvidas importantes, possuem um contexto difícil ou exigem um estudo mais aprofundado. Esses versículos ficarão separados para que o usuário possa retornar a eles posteriormente com auxílio da IA exegética.",
  },
};
  ...
  const toggle = useCallback(
    async (verse: number, color: HighlightColor) => {
      ...
      if (ex?.color === color) {
        ...
        await supabase
          .from("highlights")
          .upsert({ user_id: uid, version: "GLOBAL", book, chapter: ch, verse, color });
...
function useNotes(uid: string | null, book: string, ch: number) {
  ...
  const save = useCallback(
    async (verse: number, content: string) => {
      ...
      await supabase
        .from("verse_notes")
        .upsert({ user_id: uid, version: "GLOBAL", book, chapter: ch, verse, content });
      ...
      parts.push(
        <Popover key={match.index}>
          <PopoverTrigger asChild>
            <span
              className="text-amber-300 font-medium underline decoration-amber-300/30 decoration-dashed underline-offset-4 cursor-pointer hover:text-amber-200 transition-colors"
          ...
          <PopoverContent
            className="w-64 p-4 bg-[#141419] border-white/10 text-white shadow-2xl rounded-2xl"
            onClick={(e) => e.stopPropagation()}
          >
...
export default function Bible() {
  ...
  const [abbr, setAbbr] = useState(() =>
    typeof window !== "undefined" ? localStorage.getItem("bible.last_book") || "jo" : "jo",
  );
  const [ch, setCh] = useState(() =>
    typeof window !== "undefined" ? Number(localStorage.getItem("bible.last_chapter")) || 1 : 1,
  );
  ...
  const [fs, setFs] = useState(() =>
    typeof window !== "undefined" ? Number(localStorage.getItem("bible.font") || 18) : 18,
  );
  ...
  const [originalVerses, setOriginalVerses] = useState<
    Record<
      number,
      { text: string; notes: string | null; originalLang: string | null; keyWords: any[] | null }
    >
  >({});
  ...
  useEffect(() => {
    if (["NVI", "ACF", "AA", "ORIGINAL"].includes(translation)) {
      ...
      if (!fetchedBibles["AVE"]) {
        setLoadingTranslation(true);
        fetch(
          `https://raw.githubusercontent.com/fidalgobr/bibliaAveMariaJSON/main/bibliaAveMaria.json`,
        )
          .then((r) => r.json())
          .then((data) => {
            setFetchedBibles((prev) => ({
              ...prev,
              AVE: [...data.antigoTestamento, ...data.novoTestamento],
            }));
            setLoadingTranslation(false);
          })
          .catch((e) => {
  ...
  const syncPrefsToSupabase = useCallback(
    (userId: string, newAbbr: string, newCh: number, newTranslation: string, newFs: number) => {
      supabase
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (!prefsLoadedRef.current) {
        ...
        if (prefs) {
          ...
          if (prefs.translation && prefs.translation !== translation)
      ...
      const { error: rcErr } = await supabase
        .from("read_chapters")
        .upsert(
          { user_id: userId, book: book.name, chapter: ch },
        ...
        if (translation === "AVE") {
          ...
          const bookData = bibleData.find((b: any) => {
            const bName = b.nome
              ...
              .replace(/[\u0300-\u036f]/g, "");
            return (
              bName === normalizedName ||
              bName.includes(normalizedName) ||
              normalizedName.includes(bName)
            );
          });
          if (bookData) {
  ...
  const atBooks = BIBLE_BOOKS.filter(
    (b) => b.testament === "AT" && b.name.toLowerCase().includes(bookQ.toLowerCase()),
  );
  const ntBooks = BIBLE_BOOKS.filter(
    (b) => b.testament === "NT" && b.name.toLowerCase().includes(bookQ.toLowerCase()),
  );
  ...
  function goChapter(d: number) {
    ...
    if (next < 1) {
      ...
      if (i > 0) {
        setAbbr(BIBLE_BOOKS[i - 1].abbr);
        setCh(BIBLE_BOOKS[i - 1].chapters);
      }
    ...
    <AppShell>
      ...
      <div className="flex items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            style={{
              background: "oklch(0.55 0.18 85 / 0.2)",
        ...
        >
          <button
            onClick={() => setFs((s) => Math.max(14, s - 1))}
          ...
          <button
            onClick={() => setFs((s) => Math.min(30, s + 1))}
            className="p-1.5 rounded-lg hover:bg-white/10 text-white/60 hover:text-white transition-all"
          >
      ...
      <div className="mb-5">
        ...
        >
          ...
          <ChevronDown
            className={`w-3 h-3 transition-transform ${showLegend ? "rotate-180" : ""}`}
          />
        </button>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {(["green", "yellow", "red"] as HighlightColor[]).map((colorKey) => (
                    ...
                    >
                      <span
                        className={`w-3.5 h-3.5 rounded-full mt-0.5 shrink-0 ${COLORS[colorKey].dot}`}
                      />
                      <div>
              ...
              <button
                ...
                style={
                  c === ch
                    ? {
                        background:
                          "linear-gradient(135deg, oklch(0.65 0.18 255), oklch(0.58 0.2 280))",
                        color: "white",
                ...
                onMouseEnter={(e) => {
                  if (c !== ch) (e.target as HTMLElement).style.background = "oklch(1 0 0 / 0.08)";
                }}
                onMouseLeave={(e) => {
                  if (c !== ch) (e.target as HTMLElement).style.background = "";
                }}
              >
                {c}
              </button>
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(160deg, oklch(0.17 0.04 260 / 0.95), oklch(0.13 0.025 260 / 0.98))",
          border: "1px solid oklch(0.55 0.18 85 / 0.2)",
          boxShadow:
            "0 0 0 1px oklch(0.55 0.18 85 / 0.08) inset, 0 32px 80px -20px oklch(0.55 0.18 85 / 0.15)",
        }}
        onClick={() => {
          setSelVerse(null);
          setShowChs(false);
      ...
      >
        {/* Header */}
        <div className="mb-8">
          <p className="text-[10px] uppercase tracking-[0.35em] text-amber-400/60 mb-1">
            {translation}
          </p>
        ...
        {translation === "ORIGINAL" && (
          <div
            className="mb-6 px-4 py-3 rounded-2xl text-xs leading-relaxed"
            style={{
              background: "oklch(0.55 0.18 85 / 0.08)",
              border: "1px solid oklch(0.55 0.18 85 / 0.25)",
            }}
          >
            ...
            <p className="text-white/55">
              ...
              Toque nos versículos para visualizar notas exegéticas e detalhes dos idiomas originais
              onde disponíveis.
            </p>
          </div>
        )}
            ...
            return (
              <div key={v}>
                <p
                  onClick={(e) => {
                    e.stopPropagation();
                    setSelVerse(isSel ? null : v);
                    setShowChs(false);
                    setShowTranslations(false);
                    if (translation === "ORIGINAL" && hasLiteralTranslation)
                      setShowOriginalNote(isShowingOriginalNote ? null : v);
                  }}
                ...
                >
                  ...
                  {translation === "ORIGINAL" && originalData
                    ? parseOriginalText(displayText, originalData.keyWords)
                    : displayText}
                  {hasNote && (
                    <span className="inline-block ml-1.5 w-1.5 h-1.5 rounded-full bg-violet-400 align-middle animate-pulse" />
                  )}
                </p>
                ...
                <AnimatePresence>
                  {isShowingOriginalNote && originalData?.notes && (
                    ...
                    >
                      <div
                        className="mx-3 mb-2 p-4 rounded-2xl text-xs text-white/70 leading-relaxed"
                        style={{
                          background: "oklch(0.55 0.18 85 / 0.06)",
                          border: "1px solid oklch(0.55 0.18 85 / 0.2)",
                          borderLeft: "3px solid oklch(0.55 0.18 85 / 0.6)",
                        }}
                      >
                        <p className="text-amber-300/80 font-semibold text-[10px] uppercase tracking-wider mb-1.5">
                          📜 Nota Exegética • {originalData.originalLang}
                        </p>
        ...
        <AnimatePresence>
          {selVerse !== null && (
            ...
            >
              ...
              {(["green", "yellow", "red"] as HighlightColor[]).map((c) => (
                <button
                  ...
                  className={`w-7 h-7 rounded-full transition-all hover:scale-110 ${COLORS[c].dot} ${
                    hl.find((h) => h.verse === selVerse)?.color === c
              ...
              <button
                onClick={() => {
                  const ex = notes.find((n) => n.verse === selVerse);
                  setNoteDraft(ex?.content ?? "");
                  setNoteVerse(selVerse);
                  setSelVerse(null);
                }}
              ...
              {notebooks.length > 0 && (
                <button
                  onClick={() => {
                    setAddToNotebookVerse(selVerse);
                    setSelVerse(null);
                  }}
                  className="flex items-center gap-1.5 rounded-xl px-2.5 py-1.5 text-xs text-white/70 hover:bg-white/10 hover:text-white transition-all"
                >
      ...
      {notes.length > 0 && (
        <div
          ...
          style={{
            background:
              "linear-gradient(160deg, oklch(0.15 0.03 260 / 0.9), oklch(0.12 0.02 260 / 0.95))",
            border: "1px solid oklch(0.55 0.18 85 / 0.2)",
          ...
          <>
            ...
            >
              ...
              <div className="overflow-y-auto flex-1 space-y-4 pr-1">
                {atBooks.length > 0 && (
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.3em] text-amber-400/50 mb-2 px-1">
                      Antigo Testamento
                    </p>
                        ...
                        <button
                          ...
                          style={
                            b.abbr === abbr
                              ? {
                                  background:
                                    "linear-gradient(135deg, oklch(0.55 0.18 85 / 0.3), oklch(0.48 0.2 70 / 0.2))",
                ...
                {ntBooks.length > 0 && (
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.3em] text-blue-400/50 mb-2 px-1">
                      Novo Testamento
                    </p>
                    <div className="grid grid-cols-2 gap-1.5">
                      {ntBooks.map((b) => (
                        <button
                          ...
                          style={
                            b.abbr === abbr
                              ? {
                                  background:
                                    "linear-gradient(135deg, oklch(0.65 0.18 255 / 0.3), oklch(0.58 0.2 280 / 0.2))",
              ...
              {showHistory ? (
                ...
                <>
                  ...
                  <div className="mt-4 flex justify-end gap-2">
                    ...
                    <button
                      onClick={async () => {
                        await saveNote(noteVerse, noteDraft);
                        setNoteVerse(null);
                        setShowHistory(false);
                      }}
                      ...
                      style={{
                        ...
                        boxShadow: "0 4px 20px oklch(0.58 0.2 280 / 0.4)",
                      }}
                    >
              ...
              {quizStep < quizQuestions.length ? (
                      ...
                      {hasSubmittedAns && (
                        <p className="text-xs text-white/50 mt-1">
                          ...
                          <span className="text-emerald-400 font-semibold">
                            {quizQuestions[quizStep].correctAnswer}
                          </span>
                        </p>
                      )}
                    </div>
                  ...
                  {hasSubmittedAns && (
                    ...
                    >
                      <p className="font-semibold text-white">
                        ...
                        quizQuestions[quizStep].correctAnswer.toLowerCase().trim()
                          ? "🎉 Resposta Correta!"
                          : "❌ Resposta Incorreta!"}
                      </p>
                      <p className="leading-relaxed">{quizQuestions[quizStep].explanation}</p>
                      <p className="text-[10px] text-white/40 font-mono">
                        Leitura sugerida: {quizQuestions[quizStep].suggestedReading}
                      </p>
                    </motion.div>
                  )}
                ...
                <div className="space-y-4 text-center py-4">
                  ...
                  <p className="text-xs text-white/50">
                    ...
                    <span className="text-white font-bold">{quizQuestions.length}</span> perguntas
                    do capítulo.
                  </p>
            ...
            >
              ...
              <p className="text-xs text-white/50 mb-4">
                ...
                <span className="text-amber-300 font-semibold">
                  {book.name} {ch}:{addToNotebookVerse}
                </span>
                <br />
                      ...
                      const content = {
                        ...
                        content: [
                          ...
                          },
                          {
                            type: "blockquote",
                            content: [
                              { type: "paragraph", content: [{ type: "text", text: verseText }] },
                            ],
                          },
